...
def expression_depth(expr):
    ...
    nested operations."""
    return 0 if type(expr) is not list else max(map(expression_depth, expr)) + 1
...
def remove_from_string(string, letters):
    ...
    return "".join([l for l in string if l not in letters])
...
def tally_letters(string):
    ...
    return dict([(s,string.count(s)) for s in set(string)])
...
def sort_points_by_Y(list_of_points):
    ...
    def getYFun(point):
        return point.getY()
    return sorted(list_of_points, key=getYFun, reverse=True)
...
def furthest_right_point(list_of_points):
    ...
    def getXFun(point):
        return point.getX()
    return max(list_of_points, key=getXFun)
