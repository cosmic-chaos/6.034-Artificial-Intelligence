...
sibling_rule = IF( AND("parent (?z) (?x)", "parent (?z) (?y)", NOT ("self (?x) (?y)")), THEN ("sibling (?x) (?y)", "sibling (?y) (?x)") )
child_rule = IF( AND("parent (?y) (?x)"), THEN ("child (?x) (?y)") )
grandparent_rule = IF( AND("parent (?x) (?z)", "parent (?z) (?y)"), THEN ("grandparent (?x) (?y)") )
grandchild_rule = IF( AND("parent (?x) (?z)", "parent (?z) (?y)"), THEN ("grandchild (?y) (?x)") )
cousin_rule = IF( AND("grandchild (?x) (?z)", "grandchild (?y) (?z)", NOT ("sibling (?x) (?y)"), NOT ("self (?x) (?y)")), THEN ("cousin (?x) (?y)", "cousin (?y) (?x)") )

# Add your rules to this list:
family_rules = [self_rule, child_rule, sibling_rule, grandparent_rule, grandchild_rule, cousin_rule ]
...
NAME = "Katya Bezugla"
COLLABORATORS = "JUlie, Tara"
HOW_MANY_HOURS_THIS_LAB_TOOK = 5
WHAT_I_FOUND_INTERESTING = "Creating the relations"
